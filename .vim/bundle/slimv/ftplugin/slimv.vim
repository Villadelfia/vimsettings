...
function! SlimvSwankCommand()
    ...
    if cmd != ''
        ...
        elseif g:slimv_osx
            return '!osascript -e "tell application \"iTerm\"" -e "if the (count of terminals) is greater than or equal to 1 then" -e "tell the first terminal" -e "set s to (make new session at the end of sessions)" -e "tell s" -e "exec command \"/usr/local/bin/' . cmd . '\"" -e "end tell" -e "end tell" -e "else" -e "set t to (make new terminal)" -e "tell t" -e "set s to (make new session)" -e "tell s" -e "exec command \"/usr/local/bin/' . cmd . '\"" -e "end tell" -e "end tell" -e "end if" -e "end tell"'
        elseif $STY != ''
